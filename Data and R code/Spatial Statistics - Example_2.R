...
# This script can be used in alternative to the one named
# "Spatial Statistics - Example" for MAC users who have problems
# with the package tmap
...
q.inc<-quantile(Silc.Spat$LogIncome,
                probs = c(0,0.2, 0.4,0.6,0.8,1))
breaks<-c(as.numeric(q.inc))

#assing col at each class
# choose a palette with gradation of colors 
plotclr <- brewer.pal(5,"Reds") 
# Defining the variable divided in categories according to quantiles
Silc.Spat$classInc <- cut(Silc.Spat$LogIncome, breaks = breaks, 
                          right = F, include.lowest=T)  
spplot(Silc.Spat, "classInc", col.regions=plotclr, 
       main="",
       colorkey = list(space="bottom",height = 1, 
                       labels = list(at = seq(0.5, length(breaks) -0.5), 
...
q.Ii<-quantile(moran.map$Ii,
                probs = c(0,0.2, 0.4,0.6,0.8,1))
breaks.Ii<-c(as.numeric(q.Ii))
plotclr.Ii <- brewer.pal(5,"Spectral") 
# Defining the variable divided in categories according to quantiles
moran.map$class.Ii <- cut(moran.map$Ii, breaks = breaks.Ii, 
                          right = F, include.lowest=T)  
spplot(moran.map, "class.Ii", col.regions=plotclr.Ii, 
       main="",
       colorkey = list(space="bottom",height = 1, 
                       labels = list(at = seq(0.5, length(breaks.Ii) -0.5), 
                                     labels = round(breaks.Ii,3))))
...
Silc.Spat$quadrant.f<-as.factor(Silc.Spat$quadrant)
spplot(Silc.Spat, "quadrant.f", col.regions=colors,
       main="",
       colorkey = list(space="bottom",height = 1))
...
plotclr.res <- rev(brewer.pal(7,"RdBu")) 
Silc.Spat$class.res <- cut(Silc.Spat$std.res.ols, breaks = my_breaks, 
                          right = F, include.lowest=T)  
spplot(Silc.Spat, "class.res", col.regions=plotclr.res, 
       main="",
       colorkey = list(space="bottom",height = 1, 
                       labels = list(at = seq(0.5, length(my_breaks) -0.5), 
...
breaks.g<-c(as.numeric(quantile(gwr.map$gender,
               probs = c(0,0.2, 0.4,0.6,0.8,1))))
breaks.g1<-c(as.numeric(quantile(gwr.map$gender.1,
                                probs = c(0,0.2, 0.4,0.6,0.8,1))))

plotclr.g <- brewer.pal(5,"YlOrRd")
plotclr.g1 <- brewer.pal(5,"YlGn")

gwr.map$class.g <- cut(gwr.map$gender, breaks = breaks.g, 
                       right = F, include.lowest=T) 
gwr.map$class.g1 <- cut(gwr.map$gender.1, breaks = breaks.g1, 
                       right = F, include.lowest=T)  

map1 <-
spplot(gwr.map, "class.g", col.regions=plotclr.g, 
       main="",
       colorkey = list(space="bottom",height = 1, 
                       labels = list(at = seq(0.5, length(breaks.g) -0.5), 
                                     labels = round(breaks.g,3))))

map2 <-
  spplot(gwr.map, "class.g1", col.regions=plotclr.g1, 
         main="",
         colorkey = list(space="bottom",height = 1, 
                         labels = list(at = seq(0.5, length(breaks.g1) -0.5), 
                                       labels = round(breaks.g1,3))))
if(!require('gridExtra')) install.packages('gridExtra', dependencies=T); library('gridExtra')
grid.arrange(spplot(gwr.map, "class.g", col.regions=plotclr.g, 
                    main="",
                    colorkey = list(space="bottom",height = 1, 
                                    labels = list(at = seq(0.5, length(breaks.g) -0.5), 
                                                  labels = round(breaks.g,3))))
